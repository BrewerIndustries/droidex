...
export function BackupReminder({
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-[11px] leading-relaxed text-zinc-300">{line}</p>
      ...
      <div className="mt-3 flex gap-2">
        <button
          ...
          className="flex-1 rounded border border-cyan-600 bg-cyan-500/10 px-2 py-1 text-[10px] font-bold tracking-wider text-cyan-300 hover:bg-cyan-500/20"
        >
          DOWNLOAD BACKUP
        </button>
        <button
          type="button"
          onClick={onSnooze}
          className="rounded border border-zinc-700 px-2 py-1 text-[10px] tracking-wider text-zinc-500 hover:text-zinc-300"
        >
          LATER
        </button>
      </div>
